...
1. mongodump --db=nodeJsDB --archive=./nodeJsDB_YYYY-MM-DD.gzip --gzip
2. mongorestore --db=nodeJsDB --archive=./nodeJsDB_YYYY-MM-DD.gzip --gzip
...
const DB_NAME_TIME = `${DB_NAME}_${(new Date().toJSON().slice(0,10))}`
const ARCHIVE_PATH = path.join(__dirname, `${DB_NAME_TIME}.gzip`);
